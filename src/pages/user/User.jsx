...
const User = () => {
  ...
  const fetchPublishedAds = async (pageNumber = page, pageSize = 8) => {
    try {
      const response = await api.get(
        `ads/my?page=${pageNumber}&size=${pageSize}`
      );
  ...
  const handleUserSubmit = async () => {
    const userData = {
      ...
      birthdate: userInfo.birthdate
        ? userInfo.birthdate.format("YYYY-MM-DD")
        : null,
    };
  ...
  const fetchTransactions = async (transactionPageNumber = transactionPage) => {
    try {
      const response = await api.get(
        `transactions/my?page=${transactionPageNumber}&size=5`
      );
      ...
      setTransactionPage(data.currentPage);
    } catch (error) {
  ...
  return (
    <div className="user-container">
                  ...
                  <tr key={ad.adId} className="ad-row">
                    ...
                    <td>
                      ...
                      {ad.status === "QUEUED_FOR_POST" && (
                        <button
                          className="post-ad"
                          onClick={() => handleFinalApproval(ad.adId)}
                        >
                          Post Ads
                        </button>
                      )}
      ...
      {transactionModalVisible && (
        ...
        >
          <h3 className="transaction-modal-header">Transaction History</h3>
          ...
          <div className="pagination-container">
            <button
              ...
              onClick={handleTransactionPreviousPage}
              ...
              <div className="plan-details">
                ...
                <p>
                  ...
                  <span className="priority-badge">
                    {selectedPlan.adPlacementPriority}
                  </span>
                </p>
                <p>
            ...
            <ul>
              {plan.map((item) => (
                <li
                  ...
                  className={`plan-item ${
                    selectedPlan?.planId === item.planId ? "selected" : ""
                  }`}
                >
      ...
      {renewModalVisible && (
        ...
        >
          ...
          <button
            className="modal-button modal-button-cancel"
            onClick={() => setRenewModalVisible(false)}
          >
            Cancel
          </button>
        </Modal>
